// components/Production/EditDishModal.jsx
import { useState } from "react";
import { X, Plus, Trash2, Sparkles, AlertTriangle } from "lucide-react";
import { askAI } from "../../services/aiService";

export default function EditDishModal({ dish, stock, onClose, onSave }) {
  const [name, setName] = useState(dish.name);
  const [category, setCategory] = useState(dish.category);
  const [yieldUnit, setYieldUnit] = useState(dish.yieldUnit);
  const [recipe, setRecipe] = useState(dish.recipe.map((r) => ({ ...r })));
...
Suggest: 1) Any unit conversions needed, 2) A recommended waste buffer % for fresh ingredients, 3) Any missing key ingredients for a typical ${name}. Be brief (max 80 words).`;
  ...
  const handleSave = () => {
    if (!name.trim()) return;
    const updatedDish = {
      ...dish, // preserve id and any other fields
      name,
      category,
      yieldUnit,
    ...
    <div className="inv-modal-overlay">
      <div className="inv-modal" style={{ maxWidth: 640 }}>
        <div className="inv-modal-header">
          <h2 className="inv-modal-title">Edit Dish Recipe</h2>
        ...
        <div className="inv-modal-body">
          <div className="prod-form-row">
            ...
            <div className="prod-form-group">
              ...
              >
                {["Coffee", "Non-Coffee", "Pastry", "Food", "Beverage"].map((c) => (
                  ...
                  <tr key={i}>
                    ...
                    <td>
                      <input
                        ...
                        max={50}
                        placeholder="0"
                        onChange={(e) => updateIngredient(i, "wasteBuffer", e.target.value)}
                      />
                    </td>
                    <td>
                      <button className="prod-remove-btn" onClick={() => removeIngredient(i)}>
                        <Trash2 size={14} />
                      </button>
                    </td>
                  </tr>
          ...
          {missingIngredients.length > 0 && (
            <div className="prod-missing-warning">
              ...
              <span>
                ...
                <strong>{missingIngredients.map((r) => r.ingredient).join(", ")}</strong>.
